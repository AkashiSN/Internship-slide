...
%% タイトル %%
\title[インターン報告会]{インターン報告会2018}%[略タイトル]{タイトル}
\author[西]{西\ 総一朗}%[略名前]{名前}
\institute[4EJ]{明石高専4年電気情報工学科}%[略所属]{所属}
% \newcommand{\todaye}{\the\year 年 \the\month 月\the\day 日}
\date{\today}%日付
...
\begin{document}
	...
	\label{sec:会社概要}
		...
		\begin{frame}\frametitle{CTF\,Kitとは}
			...
			\begin{alertblock}{CTFの準備、これひとつ。}
				\begin{itemize}
					\item スコアサーバ」と「可視化サーバ」を内包した情報セキュリティ教育教材
				\end{itemize}
			\end{alertblock}
			\begin{block}{導入実績}
		...
		\begin{alertblock}{活動内容}
			\begin{itemize}
				...
				\item セキュリティコントストの運営業務
					\begin{itemize}
						\item 佐世保高専で実施された高専セキュリティコントスト2018
						\item NTTネオメイト様で実施されたセキュリティコンテスト
					\end{itemize}
	...
	\section{ご清聴ありがとうございました} % (fold)
	\label{sec:ご清聴ありがとうございました}
	% section ご清聴ありがとうございました (end)
\end{document}